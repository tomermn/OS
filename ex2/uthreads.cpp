...
class Thread {
 ...
 public:
  ...
  void set_state(int);
  //char get_stack() const; // is it necessary?
};
...
std::unordered_map<int, int> status_map; // key is thread id, val is status (used, unused)
std::unordered_map<int, Thread *> active_threads; // key is thread id, val is ptr to thread
std::set<int> blocked_threads; // set of all thread id of blocked threads
std::unordered_map<int, int> sleeping_threads; // key is thread id, val is num of quantums if sleepeing.
std::list<int> ready_q; // thread id of all the threads in the ready queue
int running_thread;
...
/**
 * Perform a context switch once the current quantum has finished.
 */
void preemption_context_switch(int);
...
void awake_thread(int tid) {
  ...
  if (blocked_threads.find(tid) == blocked_threads.end()) {//thread is not blocked. turn it to ready
...
class Scheduler {
 ...
 public:
  ...
  void set_timer() {
    sa.sa_handler = &preemption_context_switch;
...
void forced_context_switch(int to_sleep = -1, bool to_block = false, bool to_terminate = false) {
  mask_timer_signal();
  ...
  if (ret_val == 0) {
      if (to_block) {
          ...
          status_map[prev_thread] = UNUSED;
        } else {
          active_threads[prev_thread]->set_state(READY);
          ready_q.push_back(prev_thread); // push the prev_running to ready q
        }
...
void preemption_context_switch(int sig) {
  mask_timer_signal();
  total_quantum_num++;
  reduce_sleep_quantums();
  int prev_thread = running_thread;
...
int uthread_terminate(int tid) {
  ...
  if (active_threads[tid]->get_state() == RUNNING) {
      forced_context_switch(-1, false, true);
      return EXIT_SUCCESS;
    }
...
int uthread_block(int tid) {
  ...
  if (tid == 0) {
      std::cerr << "thread library error: main thread cant be blocked" << std::endl;
      unmask_timer_signal();
      return -1;
    }

  if (active_threads[tid]->get_state() == READY) {// swap ready and running
      ready_q.remove(tid);
    }

  if (active_threads[tid]->get_state() == RUNNING) {
      forced_context_switch(-1, true, false);
      return EXIT_SUCCESS;
    }
...
int uthread_resume(int tid) {
  ...
  if ((curr_state == BLOCKED) and is_sleeping) // tid thread blocked and sleeping
    {
...
int uthread_sleep(int num_quantums) {
  mask_timer_signal();
  if (running_thread == 0) {
      std::cerr << "thread library error: main thread cant sleep" << std::endl;
      unmask_timer_signal();
      return -1;
    }
  if (num_quantums <= 0) {
      std::cerr << "thread library error: Quantums number must be positive" << std::endl; //TODO - is it?
      unmask_timer_signal();
      return -1;
    }
  forced_context_switch(num_quantums);
  unmask_timer_signal();
  return EXIT_SUCCESS;
}
...
int uthread_get_quantums(int tid) {
  mask_timer_signal();
  if (active_threads.find(tid) == active_threads.end()) {
      std::cerr << "thread library error: Thread id is not exist" << std::endl;
      unmask_timer_signal();
      return -1;
    }
